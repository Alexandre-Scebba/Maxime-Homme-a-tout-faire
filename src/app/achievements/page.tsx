...
export default function Achievements() {
  ...
  const categories: { id: string; titleKey: string; imgs: string[] }[] = [
    ...
    },
    {
      ...
      imgs: [
        ...
        "/Wall Repair/bulkhead-3-panel.jpg",
        "/Wall Repair/ceiling-3-panel.jpg",
        "/Wall Repair/lights-2-panel.jpg",
        "/Wall Repair/shower-3-panel.jpg",
        "/Wall Repair/toilet-3-panel.jpg",
      ],
    },
    {
      id: "paint",
  ...
  const open = useCallback(
    (catIdx: number, imgIdx: number) => setSelected(flatIndex(catIdx, imgIdx)),
    []
  );
    ...
    <main className="min-h-screen bg-white py-16 px-4">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {cat.imgs.map((src, ii) => (
                <button
                  key={src + ii}
                  onClick={() => open(ci, ii)}
                  className="block bg-gray-100 rounded-md overflow-hidden focus:outline-none focus:ring-2 focus:ring-sky-500"
                >
                  <img
                    src={src}
                    alt={`Project ${ci}-${ii + 1}`}
        ...
        <div className="mt-8 text-center">
          <Link href="/#quote" className="text-sky-600 font-semibold">
            {t("contact_cta") || "Contact us about a similar project"}
          </Link>
        </div>
      </div>
    </main>
